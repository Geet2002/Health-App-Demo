  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in pb-32 px-4 sm:px-6 lg:px-8 pt-4 sm:pt-8">
      <PageHeader 
        ...
        actionButton={
          !user?.is_admin ? (
            <Link to="/blood-donation/create" className="inline-flex items-center justify-center p-2.5 sm:px-4 sm:py-2 bg-red-50 text-red-600 hover:bg-red-100 font-bold rounded-xl transition-colors shrink-0 shadow-sm" title="Request Blood">
          ...
          {requests.map(request => (
            <div key={request.id} className={`bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col ${request.status === 'fulfilled' ? 'opacity-60 border-gray-200' : 'border-red-100'}`}>
              ...
              <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex justify-between items-center gap-3">
                ...
                <Link to={`${user ? `/blood-donation/${request.id}` : '/login'}`} className={`text-[13px] font-bold bg-red-100 text-red-700 px-3 py-1.5 rounded-lg hover:bg-red-200 transition-colors shrink-0 text-center ${!user ? 'cursor-not-allowed opacity-50' : ''}`}>
